...
* For the small enum use the API from the namespace `magic_enum`, and for enum-flags use the API from the
...
* Optionally define `MAGIC_ENUM_CONFIG_FILE` i.e., in your build system, with path to header file with defined macros or
...
* Returns name from enum value as `string_view` with null-terminated string.
    ...
    * If enum value does not have name, `enum_name<value>()` occurs the compilation
      error `"Enum value does not have a name."`.
